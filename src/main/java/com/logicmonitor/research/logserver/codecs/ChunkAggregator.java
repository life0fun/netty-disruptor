...
public class ChunkAggregator implements CodecAggregator {
    ...
    public static final String CHUNK_COUNTER = name(ChunkAggregator.class, "total-chunks");
    public static final String WAITING_MESSAGES = name(ChunkAggregator.class, "waiting-messages");
    public static final String COMPLETE_MESSAGES = name(ChunkAggregator.class, "complete-messages");
    public static final String EXPIRED_MESSAGES = name(ChunkAggregator.class, "expired-messages");
    public static final String EXPIRED_CHUNKS = name(ChunkAggregator.class, "expired-chunks");
    public static final String DUPLICATE_CHUNKS = name(ChunkAggregator.class, "duplicate-chunks");
    ...
    @Inject
    public ChunkAggregator(@Named("daemonScheduler") ScheduledExecutorService scheduler, MetricRegistry metricRegistry) {
    ...
    public Result addChunk(ChannelBuffer buffer) {
        ...
        final ChunkMessage msg = new ChunkMessage(readable);
     ...
     * @return null or a RawMessage raw message object
     */
    @Nullable
    private ChannelBuffer checkForCompletion(ChunkMessage gelfMessage) {
        ...
        final ChunkMessageChunk chunk = new ChunkMessageChunk(gelfMessage, null); // TODO second parameter
        ...
        if (chunkWatermark == sequenceCount) {
            ...
            for (int i = 0; i < entry.payloadArray.length(); i++) {
                final ChunkMessageChunk messageChunk = entry.payloadArray.get(i);
    ...
    private String humanReadableChunkMap() {
        ...
        for (final Map.Entry<String, ChunkEntry> entry : chunks.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().payloadArray.length(); i++) {
                final ChunkMessageChunk chunk = entry.getValue().payloadArray.get(i);
    ...
    static class ChunkEntry implements Comparable<ChunkEntry> {
        ...
        protected final AtomicReferenceArray<ChunkMessageChunk> payloadArray;
